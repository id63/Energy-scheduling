...
def testForImprovements3(solution):
    """
    Using a while loop instead of if and else statements, swaps a timing for a phase of the appliance's schedule that makes the solution 
    cheaper.
    """
    improved_solutions = [solution] #this is so it doesnt have an empty list if there are no improvements
    best_cost = solution.cost
...
def findBestSolutions(solutions):
    bestCost = 9999999999999
    bestSolutions = []
    for i in solutions:
        if i.cost < bestCost:
